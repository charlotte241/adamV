...
def main():
    ...
    for it in items:
        ...
        # Times attended counts every ticket they have ever bought, in person
        # or online, whether or not they walked through the door. Confirmed
        # attendance still drives First visit, Last attended and the grid.
        ticketed = sorted(e for e in states if e <= today)
        total_attended += len(past_came)
        total_booked += sum(1 for e, s in states.items()
                            if s != m.ATTENDED and e <= today)

        want = {
            "first": past_came[0] if past_came else "",
            "times": str(len(ticketed)),
