...
Two columns per event:
    ...
    "Jan 2024"        Attended Face to face | Booked Face to face | Booked Zoom | Didn't book
...
def main():
    ...
    m.note(f"Event grid: {len(updates)} people updated across {len(usable)} events "
           ...
           f"Attended {tally[m.ATTENDED]:,} · Booked face to face {tally[m.BOOKED_F2F]:,} · "
           f"Booked zoom {tally[m.BOOKED_ZOOM]:,}.")
    return 0
