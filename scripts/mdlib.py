...
grid, the attendance counts and the lifecycle can never disagree with each
other.
"""
...
def attendance_index(orders, today):
    ...
    per_person: {email hash: {event date: "Attended" | "Booked"}}
                an event the person holds a ticket for but which was never
    ...
    Attended  someone scanned them in, or they held an online seat. Online
              seats are never scanned because there is no door to walk through,
              so a ticket is the only evidence we will ever have for them.
    Booked    they hold a ticket and did not turn up, or the night has not
              happened yet. Both are honestly "booked and it has not become an
              attendance", which is what the board should say.
    absent    the night was never scanned at all. We cannot tell who came, so
              we say nothing instead of inventing an answer.
    """
    ...
    rank = {BOOKED: 1, ATTENDED: 2}
    per = collections.defaultdict(dict)
    for o in orders:
        e, h = o.get("edate"), o.get("h")
        if not e or not h:
            continue
        if o.get("ci") or o.get("zoom"):
            state = ATTENDED
