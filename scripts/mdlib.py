...
def attendance_index(orders, today):
    ...
    per_person: {email hash: {event date: one of the four states above}}
                on a night nobody worked the door there is no scan to
                find, so a ticket is taken as attendance rather than
                punishing people for our own process.
    ...
    for o in orders:
        ...
        if o.get("ci"):
            ...
            state = BOOKED_ZOOM
        elif e < today and not scanned.get(e) and in_person[e] > 0:
            # The door was never worked that night, so there is no scan to find.
            # Treating everyone as a no-show would punish them for our process,
            # so they are taken at their word: they bought a ticket and came.
            # Strictly BEFORE today: tonight's event has not been scanned yet
            # either, and nobody has attended a meet that has not happened.
            state = ATTENDED
        else:
            state = BOOKED_F2F
