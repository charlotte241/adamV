...
what they typed at checkout. Nothing here is a guess, a band or a "usually".
    ...
    What they have bought  every ticket they have paid for
...
personal one the next gets one continuous receipt rather than half of it.
...
anything a human typed wins. The rest are derived facts and are kept in step
with Eventbrite.
"""
...
def lifecycle(past, future, today):
    """Matches the descriptions written on the labels in monday exactly."""
    if not past:
...
def bought_text(orders):
    """One line per distinct ticket, newest first, so the cell reads like a
    receipt rather than a code."""
    lines, seen = [], set()
...
def main():
    ...
    for it in items:
        ...
        if orders:
            ...
            want["bought"] = bought_text(orders)
        ...
        town = next((cities[m.ident_hash(a)] for a in addrs
                     if m.ident_hash(a) in cities), None)
        if town:
            want["city"] = town
    ...
    m.write(updates)
    m.note(f"Enrichment: {len(updates)} rows updated ({multi} combined across "
           f"several addresses).")
    return 0
