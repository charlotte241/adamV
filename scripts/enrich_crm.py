...
def business_site(email):
    ...
    us where they work, so no inference is involved."""
    d = domain(email)
...
def main():
    ...
    for it in items:
        ...
        town = cities.get(m.ident_hash(email))
        if town:
            want["city"] = town
        ...
        if site:
            want["web"] = {"url": site, "text": domain(email)}
        else:
    ...
    m.write(updates)
    m.note(f"Enrichment: {len(updates)} rows updated.")
    return 0
