...
A monday text cell stops at 2,000 characters, which silently cut the oldest
tickets off anyone who has been coming for years - the people you least want to
lose the history of. An update has no such limit, and the ticket-by-ticket
detail was never something you could filter or sort on anyway. What you can
filter on - how many, how much, when, and which events - is in the columns.

The update is written once and edited in place afterwards, and only when the
person's orders have actually changed, so runs stay quiet and quick.
...
def history_body(orders, addrs, used):
    """Every ticket they have paid for, newest first. No truncation."""
    lines, seen = [], set()
    for o in sorted(orders, key=lambda x: x["dt"], reverse=True):
        ...
        line = f"{when}: {o['qty']} x {what}, GBP {o['gross']:.2f}"
        if line not in seen:
            seen.add(line)
            lines.append(line)
    ...
    head = (f"<b>{MARKER}</b><br>{len(lines)} tickets, {span}, GBP {total:.2f} in total")
    if used > 1:
        head += f" — combined across {used} email addresses"
    return head + "<ul>" + "".join("<li>" + x + "</li>" for x in lines) + "</ul>"
...
def main():
    ...
    m.note(f"Enrichment: {len(updates)} rows updated ({multi} combined across several "
           f"addresses). Ticket history: {new} written, {ed} revised.")
    return 0
